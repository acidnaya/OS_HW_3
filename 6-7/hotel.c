...
int check_rooms(int gender, int num) {
    ...
    bzero(buffer, 1024);
    sprintf(buffer, "A %s #%d came to the hotel...\n", (gender == 2 ? "woman" : "man"), num);
    send_message(view_socket);
    ...
    for (int i = 0; i < size; ++i) {
        if (i < double_rooms && (addr[i] == gender || addr[i] == 0)) {
            ...
            bzero(buffer, 1024);
            sprintf(buffer, "A %s #%d entered double room #%d...\n", (gender == 2 ? "woman" : "man"), num, i + 1);
            send_message(view_socket);
            sem_post(clients);
            return i + 1;
        }
        if (i >= double_rooms && addr[i] == 0) {
            ...
            close(shm);
            bzero(buffer, 1024);
            sprintf(buffer, "A %s #%d entered single room #%d...\n", (gender == 2 ? "woman" : "man"), num, i + 1);
            send_message(view_socket);
            sem_post(clients);
            return i + 1;
        }
    ...
    bzero(buffer, 1024);
    sprintf(buffer, "A %s #%d left hotel\n", (gender == 2 ? "woman" : "man"), num);
    send_message(view_socket);
    close(shm);
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    view_socket = accept_connection(view_addr, addr_size);
    pid_t childpid;
